        ...
        try {
            ...
            if (!user) {
                ...
                bcrypt.compare(req.body.password, user.password, (error, result) => {
                    if (error) {
                        return res.status(500).json({ message: 'Internal Server Error' }); // Return error if password comparison fails
                    }
                    if (result) {
                        // Generate JWT token if password matches
                        const token = jwt.sign({ email: user.email, name: user.name }, user.type === 0 ? "USER" : "ADMIN");
                        // Return success message, user type, email, and token
                        return res.status(200).json({
                            message: user.type === 0 ? "User logged in" : "Admin logged in",
                            type: user.type,
                            email: user.email,
                            token: token
                        });
